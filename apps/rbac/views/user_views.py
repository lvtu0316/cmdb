...
from ..serializer.user_serializer import UserListSerializer, UserCreateSerializer, UserModifySerializer, \
    UserAvatarSerializer
from ..models import Menu, UserProfile
from ..serializer.menu_serializer import MenuSerializer
from untitled2.basic import ApiResponse
from untitled2.schema_view import CustomSchema
from untitled2.settings import SECRET_KEY
from untitled2.status_code import *
...
class UserAuthView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user:
            ...
            return ApiResponse({'token': jwt.encode(payload, SECRET_KEY)}, status=OK)
        else:
            return ApiResponse('用户名或密码错误！', status=BAD)
...
class UserMenuView(APIView):
    ...
    def get(self, request):
        if request.user.id is not None:
            ...
            return ApiResponse(menu_data, status=OK)
        else:
            return ApiResponse('请登录后访问!',status=FORBIDDEN)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if check_password(old_password, user.password):
            ...
            if new_password1 == new_password2:
                ...
                user.save()
                return ApiResponse('密码修改成功!')
            else:
                return ApiResponse('新密码两次输入不一致!', status=BAD)
        else:
            return ApiResponse('旧密码错误!', status=BAD)
...
class UserUploadImage(APIView):
    ...
    def post(self, request, *args, **kwargs):
        instance = UserAvatarSerializer(data=request.data, instance=request.user)
        if instance.is_valid():
            instance.save()
            return ApiResponse('上传成功!')
        else:
            return ApiResponse(instance.errors, status=BAD)
...
class UserViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # serializer.save(password='123456')
        self.perform_create(serializer)
        headers = self.get_success_headers(serializer.data)
        return ApiResponse(serializer.data, status=CREATED, headers=headers)
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if 'admin' in perms or 'user_all' in perms or request.user.is_superuser:
            try:
                ...
                pass
            return ApiResponse(status=NO_CONTENT)
